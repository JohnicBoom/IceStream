...
- **Locate:** On panel open, use Omarchy weather.json `{name, latitude, longitude}` when both coords exist (the name is only a label; NWS needs lat/lon). Typed US ZIP still wins. No city-name geocoder. Then `api.weather.gov/points/{lat},{lon}` → `properties.nwr.transmitter`. That is the **covering** dish, not haversine-nearest. Then `GET /radio/{callSign}` for metadata. `GET /points/…/radio` is SSML forecast speech, **not** the radio stream.
