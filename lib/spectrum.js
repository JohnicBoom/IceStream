...
function relativeBands(bands, floor) {
  ...
  var thresh = floor === undefined || floor === null ? 0.08 : Number(floor)
  if (!isFinite(thresh) || thresh <= 0) thresh = 0.08
  if (max < thresh) {
    for (i = 0; i < n; i++) out[i] = out[i] * 0.2 / thresh
    return out
  }
  for (i = 0; i < n; i++) out[i] = out[i] / max
  return out
}

function displayPeak(rawPeak, volumePercent) {
  var raw = Number(rawPeak)
  if (!isFinite(raw) || raw < 0) raw = 0
  var vol = Number(volumePercent)
  if (!isFinite(vol) || vol < 1) return raw
  if (vol > 100) vol = 100
  return raw * (100 / vol)
}
