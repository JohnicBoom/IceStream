...
function isPlayingCallSign(state, callSign) {
  var current = state || initialState()
  if (current.status !== "playing" && current.status !== "connecting" && current.status !== "paused")
    return false
  if (!current.station || !callSign) return false
  return String(current.station.callSign).toUpperCase() === String(callSign).toUpperCase()
}
...
var api = {
  ...
  isPlayingCallSign: isPlayingCallSign
}
